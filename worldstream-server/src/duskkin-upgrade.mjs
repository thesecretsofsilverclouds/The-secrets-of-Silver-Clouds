...
import { initialDuskkinComplianceState } from './duskkin-compliance.mjs';
...
const FROM = 'canon-ambient-p183-v25', TO = 'canon-ambient-p183-v26';
...
export function upgradeDuskkinCompliance({ directory, backupPath } = {}) {
  ...
  if (RULES_VERSION !== TO) throw new Error('This upgrade belongs to release v26 only');
  ...
  try {
    ...
    if (matchesRulesIdentity(before.rules_version, fixture, TO)) {
      ...
      if (![FROM, TO].includes(manifest.rulesVersion) || receipts.length !== 1
        ...
        || receipt.activationActionId !== `duskkin-v26/activate/${receipt.cutoverAt}`
        || !/^[a-f0-9]{64}$/.test(receipt.historicalLedgerDigest ?? '')
        || !/^[a-f0-9]{64}$/.test(receipt.backupSha256 ?? '')
        || !Object.hasOwn(original, 'duskkinCompliance')) throw new Error('No matching committed upgrade receipt');
    ...
    if (manifest.rulesVersion !== FROM || !matchesRulesIdentity(before.rules_version, fixture, FROM))
      throw new Error('Only the pinned v25 release can be upgraded');
    if ((original.meta.upgrades ?? []).some(item => item.to === TO)) throw new Error('Unexpected prior duskkin upgrade receipt');
    ...
    if (!Object.hasOwn(state, 'duskkinCompliance')) state.duskkinCompliance = initialDuskkinComplianceState();
    ...
    const action = { id: `duskkin-v26/activate/${cutoverAt}`, type: 'WORLD_DUSKKIN_ACTIVATE',
    ...
    if (pendingBefore.some(row => row.id === action.id || JSON.parse(row.action_json).type === action.type))
      throw new Error('Unexpected prior duskkin activation');
  ...
  function saveManifest() {
    const temporary = `${manifestPath}.v26.tmp`;
