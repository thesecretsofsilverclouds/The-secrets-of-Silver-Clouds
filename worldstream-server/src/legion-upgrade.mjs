...
import { initialLegionJobsState } from './legion-jobs.mjs';
...
const FROM = 'canon-ambient-p183-v24', TO = 'canon-ambient-p183-v25';
...
// Does not fold into the v23→v24 MEU upgrade. Does not backfill Legion jobs.
export function upgradeLegionJobs({ directory, backupPath } = {}) {
  if (typeof directory !== 'string' || !directory.trim()) throw new TypeError('An existing pinned directory is required');
  if (![TO, 'canon-ambient-p183-v26', 'canon-ambient-p183-v27', 'canon-ambient-p183-v28', 'canon-ambient-p183-v29', 'canon-ambient-p183-v30'].includes(RULES_VERSION)) throw new Error('This upgrade belongs to release v25 or later');
  ...
  try {
    ...
    if (matchesRulesIdentity(before.rules_version, fixture, TO)) {
      ...
      if (![FROM, TO].includes(manifest.rulesVersion) || receipts.length !== 1
        ...
        || receipt.activationActionId !== `legion-v25/activate/${receipt.cutoverAt}`
        || !/^[a-f0-9]{64}$/.test(receipt.historicalLedgerDigest ?? '')
        || !/^[a-f0-9]{64}$/.test(receipt.backupSha256 ?? '')
        || !Object.hasOwn(original, 'legionJobs')) throw new Error('No matching committed upgrade receipt');
    ...
    if (manifest.rulesVersion !== FROM || !matchesRulesIdentity(before.rules_version, fixture, FROM))
      throw new Error('Only the pinned v24 release can be upgraded');
    if ((original.meta.upgrades ?? []).some(item => item.to === TO)) throw new Error('Unexpected prior legion upgrade receipt');
    ...
    if (!Object.hasOwn(state, 'legionJobs')) state.legionJobs = initialLegionJobsState();
    ...
    const action = { id: `legion-v25/activate/${cutoverAt}`, type: 'WORLD_LEGION_ACTIVATE',
    ...
    if (pendingBefore.some(row => row.id === action.id || JSON.parse(row.action_json).type === action.type))
      throw new Error('Unexpected prior legion activation');
  ...
  function saveManifest() {
    const temporary = `${manifestPath}.v25.tmp`;
