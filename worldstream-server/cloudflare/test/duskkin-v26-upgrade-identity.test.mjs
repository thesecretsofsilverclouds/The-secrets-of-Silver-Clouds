...
const OLD = 'canon-ambient-p183-v25';
...
function v25Sqlite(seed = 'cf-duskkin-live-copy', { bareRules = false } = {}) {
  const directory = mkdtempSync(join(tmpdir(), 'sc-duskkin-cf-'));
  ...
  writeFileSync(join(directory, 'active-world.json'), JSON.stringify({
    format: 1, database: 'world.sqlite', rulesVersion: OLD, note: 'v25 test fixture'
  }));
  if (bareRules) rewriteBareRules(dbPath, OLD);
  return { directory, dbPath, backupPath: join(directory, 'before-v26.sqlite'), seed, startMs: start };
}
...
test('Cloudflare DO refuses an unupgraded v25 sqlite under v26 rules', () => {
...
test('after the documented v25→v26 copy-upgrade, Cloudflare DO accepts the same epoch and seed', () => {
  const live = v25Sqlite();
...
test('Cloudflare-shaped bare v25 export is refused, then accepted after copy-upgrade', () => {
  const live = v25Sqlite('cf-bare-duskkin-copy', { bareRules: true });
  const before = loadRow(live.dbPath);
